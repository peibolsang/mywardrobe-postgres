...
export default function Home() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const [wardrobeRes, schemaRes] = await Promise.all([
          fetch('/api/wardrobe'),
          fetch('/api/schema'),
        ]);

        if (!wardrobeRes.ok) throw new Error(`HTTP error! status: ${wardrobeRes.status} from /api/wardrobe`);
        if (!schemaRes.ok) throw new Error(`HTTP error! status: ${schemaRes.status} from /api/schema`);
        ...
        if (wardrobeJson.length > 0) {
          setFormData(wardrobeJson[0]);
        }
      } catch (e: any) {
        setError(e.message);
  ...
  if (wardrobeData.length === 0) return <div className="flex justify-center items-center min-h-screen">No wardrobe items found.</div>;
